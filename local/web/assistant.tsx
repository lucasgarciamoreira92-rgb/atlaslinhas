...
export default function Assistant(){
 const [mode,setMode]=useState<'draft'|'chat'|'config'>('draft');
 ...
 return <aside className="atlas-assistant">
  ...
  <section id="assistant-panel" aria-label={mode==='config'?'Configuração do assistente':'Assistente Atlas'} hidden={!open}>
   ...
   <nav aria-label="Modo do assistente"><button onClick={()=>setMode('draft')}>Preparar cadastro</button><button onClick={()=>setMode(configured?'chat':'config')}>Conversa OpenAI</button><button onClick={()=>setMode('config')}>Configurar OpenAI</button></nav>
