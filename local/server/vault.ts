...
export async function vaultPOST(req:Request){let actor=await requireActor();const input=z.object({action:z.string()}).passthrough().parse(await req.json());
 if(input.action==='unlock'){
  const d=z.object({action:z.literal('unlock'),id:z.string().max(100),password:z.string().min(1).max(128),scope:z.enum(['vault','quick','assistant']),purpose:z.enum(['read','write'])}).strict().parse(input);
  authorizedRow(actor,d.id,d.purpose==='read'?'reveal':'edit');await verifySessionPassword(req,d.password);actor=await requireActor();
  const row=authorizedRow(actor,d.id,d.purpose==='read'?'reveal':'edit'),policy=policyOf(row);if((d.scope==='quick'||d.scope==='assistant'&&d.purpose==='read')&&!policy.quickAccess)throw new AccessError('Esta credencial só pode ser consultada na seção Cofre.',403);if(d.purpose==='write'&&d.scope==='quick')throw new AccessError('Edite a credencial na seção Cofre ou no painel protegido do assistente.',403);
 ...
 const d=z.object({action:z.enum(['read','write']),id:z.string().max(100),token:z.string().regex(/^[a-f0-9]{64}$/),scope:z.enum(['vault','quick','assistant']),field:z.enum(['password','recoveryCodes']).optional(),intent:z.enum(['reveal','copy']).optional(),secret:secretWriteSchema.optional()}).strict().parse(input);
 return transaction(()=>{const row=authorizedRow(actor,d.id,d.action==='read'?'reveal':'edit'),policy=policyOf(row);if((d.scope==='quick'||d.scope==='assistant'&&d.action==='read')&&!policy.quickAccess)throw new AccessError('Abra esta credencial dentro do Cofre.',403);
