...
export const slotSchema=z.enum(slotOptions);
...
export const deviceSchema=z.object({id:z.string().min(1).max(100),name:word.min(1),type:z.enum(['iphone','android']),model:word.min(1),location:word,owner:word,slots:z.array(slotSchema).min(1,'Selecione ao menos um slot.').max(3).refine(v=>new Set(v).size===v.length,'Slots duplicados.').optional()});
...
export function newLine():Line{return {number:'',name:'',carrier:'',usage:'phone',platform:'',status:'active',deviceId:null,slot:null,location:'',owner:'',team:'',cost:null,dueDay:null,notes:'',version:0}}
