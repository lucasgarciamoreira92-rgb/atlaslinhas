...
export function lineValues(l:Line,c:Config):Values{
 ...
 return {'Número':displayNumber(l.number),'Identificação':l.name,'Operadora':l.carrier,'Uso do número':usageLabels[l.usage],'Sistema / uso virtual':l.platform,'Situação':statusLabels[l.status],'Aparelho':device?`${device.name} · ${device.model}`:l.deviceId||'','Slot / eSIM':l.slot||'','Localização do chip':locationOf(l,c),'Local guardado no cadastro':l.location,'Responsável':l.owner,'Setor':l.team,'Mensalidade':currency(l.cost),'Pacote de dados ativo':l.dataPackage||'','Dia de vencimento':l.dueDay?String(l.dueDay):'','Observação':l.notes,...(device?deviceValues(device):{})};
}
...
export function legacyValues(l:Line):Values{
 return {'Número':displayNumber(l.number),'Identificação':l.name,'Operadora':l.carrier,'Uso do número':usageLabels[l.usage],'Sistema / uso virtual':l.platform,'Situação':statusLabels[l.status],'Referência do aparelho':l.deviceId||'','Slot / eSIM':l.slot||'','Local guardado no cadastro':l.location,'Responsável':l.owner,'Setor':l.team,'Mensalidade':currency(l.cost),'Pacote de dados ativo':l.dataPackage||'','Dia de vencimento':l.dueDay?String(l.dueDay):'','Observação':l.notes};
}
