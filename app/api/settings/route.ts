...
export async function POST(req:Request){return saveSettings(req);}
export async function saveSettings(req:Request,expectedLines?:string){
 const guard=mutationGuard(req);if(guard)return guard;
 try{
  const actor=await requireActor(expectedLines!==undefined),previous=await config();
  ...
  const valid="NOT EXISTS (SELECT 1 FROM lines l WHERE l.device_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM json_each(?) d,json_each(d.value,'$.slots') s WHERE json_extract(d.value,'$.id')=l.device_id AND s.value=l.slot))"+(expectedLines===undefined?'':" AND (SELECT json_group_array(json_array(id,version)) FROM (SELECT id,version FROM lines ORDER BY id))=?");
  const bindings=expectedLines===undefined?[devices]:[devices,expectedLines];
  const write=c.version===0?db().prepare(`INSERT OR IGNORE INTO settings(id,data,version) SELECT ?,?,? WHERE ${valid}`).bind('main',payload,version,...bindings):db().prepare(`UPDATE settings SET data=?,version=? WHERE id=? AND version=? AND ${valid}`).bind(payload,version,'main',c.version,...bindings);
