...
async function accountSection(page:Page,section='Verificações e autenticações'){
 if(!await page.getByRole('navigation',{name:'Menu principal'}).isVisible())await page.getByRole('button',{name:'Toggle Sidebar',exact:true}).click();
...
async function devices(page:Page){await settings(page);for(const [type,name] of [['iPhone','iPhone da direção'],['Android','Samsung da operação']]){await page.getByRole('button',{name:'Novo aparelho',exact:true}).click();await page.getByLabel(type,{exact:true}).check();await page.getByLabel('Nome do aparelho',{exact:true}).fill(name);await page.getByLabel('Local do aparelho',{exact:true}).fill('Sala de teste');await page.getByLabel('Responsável pelo aparelho',{exact:true}).fill(type==='iPhone'?'Lucas':'Ana');await saveDevice(page);}}
...
test('verificações: cadastro pela interface, dois aparelhos, pendência e conferência',async({page,app},info)=>{
 ...
 await page.getByRole('button',{name:'Adicionar método',exact:true}).click();await page.getByLabel('Tipo de método',{exact:true}).selectOption('prompt');
 const firstDevice=await page.getByLabel('Aparelho cadastrado',{exact:true}).locator('option').filter({hasText:'iPhone da direção'}).getAttribute('value');await page.getByLabel('Aparelho cadastrado',{exact:true}).selectOption(firstDevice!);
 await page.getByLabel('Situação informada',{exact:true}).selectOption('available');
 ...
 const deviceValue=await second.getByLabel('Aparelho cadastrado',{exact:true}).locator('option').filter({hasText:'Samsung da operação'}).getAttribute('value');await second.getByLabel('Aparelho cadastrado',{exact:true}).selectOption(deviceValue!);await second.getByLabel('Situação informada',{exact:true}).selectOption('available');
...
test('cofre: senha protegida, acesso fora da seção, permissão do operador e ocultação',async({page,operatorPage})=>{
...
test('verificações: consulta pela linha e uso em janelas menores',async({page},info)=>{
 const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await setup(page);await dashboard(page);await newLine(page,'Linha de confirmação');await saveLine(page);await accountSection(page);await newAccount(page);await page.getByRole('button',{name:'Adicionar método',exact:true}).click();const value=await page.getByLabel('Linha cadastrada',{exact:true}).locator('option').filter({hasText:'Linha de confirmação'}).getAttribute('value');await page.getByLabel('Linha cadastrada',{exact:true}).selectOption(value!);await saveAccount(page);
 ...
 for(const width of [1280,768,390]){await page.setViewportSize({width,height:1000});await accountSection(page);await page.getByLabel('Buscar contas',{exact:true}).fill('Google');await page.locator('.access-list').getByRole('button',{name:/Google · Direção/}).click();await expect(page.getByRole('heading',{name:'Onde confirmar',exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);await info.attach('verificacoes-'+width,{body:await page.screenshot({fullPage:true}),contentType:'image/png'});}
